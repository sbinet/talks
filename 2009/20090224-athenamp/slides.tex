...
\title[AthenaMP]{AthenaMP \\ Parallelisation \\ d'\\Athena}
  ...
  \begin{columns}
    \begin{column}{0.9\textwidth}
        ...
        \begin{itemize}
          ...
          \item cr\'eation d'un script pour \emph{merger} les fichiers POOL et ROOT
            \begin{itemize}
              ...
              \item \emph{fallback solution:} utiliser un sous-process \texttt{Athena+RecExCommon} pour merger
                \begin{itemize}
                  \item $\Rightarrow +$ d'overhead, mais $+$ robuste (et $+$ test\'e)
                \end{itemize}
            \end{itemize}
        \end{itemize}
      \end{block}
      ...
      \begin{exampleblock}{Status}
        \begin{itemize}
          \item \emph{beta} code (int\'egr\'e dans $15.0.0$)
...
\frame{
  \frametitle{Prospects - II}

  \begin{columns}
    \begin{column}{0.9\textwidth}
      \begin{block}{Utilisation du module kernel \texttt{KSM}}
        \begin{itemize}
          \item module exp\'erimental pour efficacement partager de la m\'emoire entre processus non-apparent\'es
            \begin{itemize}
              \item pas de relation de filiation \`a la \texttt{fork}
              \item objectif: partager la m\'emoire entre applications virtualis\'ees ($n$ instances d'\texttt{apache,mod\_python,\ldots})
            \end{itemize}
          \item application d\'eclare les zones m\'emoires susceptibles d'\^etre partag\'ees
            \begin{itemize}
              \item fait automatiquement avec un \texttt{glibc/tcmalloc} modifi\'e
            \end{itemize}
          \item calcul d'un checksum pour chaque \texttt{page}
          \item \texttt{KSM} scanne \`a intervalles r\'eguliers (ou par signal de l'application) les plages m\'emoires \'etiquett\'ees
          \item merge les \texttt{page}s semblables
          \item premiers tests (\texttt{CMS \& Atlas}) montrent que l'on peut \emph{``surbooker''} la m\'emoire \`a $\sim 300\%$
        \end{itemize}
      \end{block}

      \begin{exampleblock}{}
        il est pr\'evu d'essayer d'int\'egrer le module dans SLC5 pour faciliter son d\'eploiement (CERN-IT)
        \begin{itemize}
          \item \texttt{KSM} faciliterait grandement l'implementation d'\texttt{AthenaMP} (moins invasif)
        \end{itemize}
      \end{exampleblock}
    \end{column}
  \end{columns}
